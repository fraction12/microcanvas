...
- [x] 1.2 Sanitize rendered Markdown HTML before staging it as an active surface.
...
- [x] 1.5 Preserve deterministic rendering behavior for safe HTML-like surfaces.

## 2. Native viewer hardening

- [x] 2.1 Create `WKWebView` instances with an explicit configuration for hardened local-surface presentation.
...
- [x] 3.1 Limit `WKWebView` local read access to the staged active-surface directory.
- [x] 3.2 Ensure generated or copied web surfaces only depend on assets intentionally present in that staged directory.
...
- [x] 4.2 Reject symlinked input files and symlinked ancestor-directory paths that would bypass the normal root restriction.
...
- [x] 5.1 Update README wording to describe the trusted-local-artifact security posture honestly.
- [ ] 5.2 Add or update security guidance for HTML-like surfaces and unsafe-content expectations.
- [x] 5.3 Document that Microcanvas is not a hostile-content browser sandbox.
